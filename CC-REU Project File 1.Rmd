...
#Fibonacci Number Code: We want to generate a list of Fibonacci numbers for later reference.
phi <-.5*(1+sqrt(5))
...
#Here we rewrite Fib_num_list[i] as a function. I had, at some points, used both lowercase and uppercase, and so both are defined here. 
...
```{r ConFib Definition}
#Thousand sum Definition (This was first series explored, so naming convention wasn't established yet). 
...
#Creating vectors of sums for increments of five group pairs"
...
#Joining above vectors into first results tibble, "Consec_Values_Tib_All"
options(digits = 10)
...
#Using regression to create an approximating equation of a predetermined form (discovered using Desmos and trial and error)
...
#Because of convergence, only some partial sums are plotted, for visibility. 
...
calculate_pseudo_r2 <- function(data, fitted_model) {
  ...
  rss <- sum(residuals(fitted_model)^2)  # Compute RSS
  
  response_var <- all.vars(formula(fitted_model))[1]  # Extract the response variable from the model formula
  
  tss <- sum((data[[response_var]] - mean(data[[response_var]]))^2)   # Compute TSS
  
  pseudo_r_squared <- 1 - (rss/tss)  # Compute pseudo-R2
